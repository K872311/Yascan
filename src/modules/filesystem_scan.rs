...
pub fn scan_path (
    ...
    let file_count = WalkDir::new(target_folder)
        .follow_links(false)
        .into_iter()
    ...
    // Set total files in scan state if available
    if let Some(state) = scan_state {
        state.set_files_total(file_count);
    }
    ...
    for ymatch in yara_matches.iter() {
        if !sample_matches.is_full() {
            ...
            sample_matches.insert(sample_matches.len(), GenMatch{
                ...
                rule_file: ymatch.rule_file.clone(),
                rule_path: ymatch.rule_path.clone(),
            });
        }
    }
    ...
    if !sample_matches.is_empty() {
        ...
        let shown_reasons: Vec<MatchReason> = sample_matches.iter().take(reasons_to_show)
            .map(|m| MatchReason { 
                ...
                rule_file: m.rule_file.clone(),
                rule_path: m.rule_path.clone(),
            })
            .collect();
        ...
        Ok(scan_results) => {
            ...
            for matching_rule in scan_results.matching_rules() {
                if !yara_matches.is_full() {
                    ...
                    let mut rule_file: Option<String> = None;
                    let mut rule_path: Option<String> = None;
                    ...
                    for (key, value) in matching_rule.metadata() {
                        match key {
                            ...
                            "rule_file" => {
                                match value {
                                    yara_x::MetaValue::String(s) => rule_file = Some(s.to_string()),
                                    _ => {}
                                }
                            }
                            "rule_path" => {
                                match value {
                                    yara_x::MetaValue::String(s) => rule_path = Some(s.to_string()),
                                    _ => {}
                                }
                            }
                            _ => {}
...
mod tests {
    ...
    mod program_directory_exclusion_tests {
        ...
        fn test_program_dir_match() {
            let program_dir = "/opt/yascan";
            let file_path = Path::new("/opt/yascan/signatures/rules.yar");
            let program_dir_path = Path::new(program_dir);
        ...
        fn test_program_dir_exact_match() {
            let program_dir = "/opt/yascan";
            let file_path = Path::new("/opt/yascan");
            let program_dir_path = Path::new(program_dir);
            assert!(file_path.starts_with(program_dir_path));
        }
        ...
        fn test_program_dir_no_match_similar_prefix() {
            let program_dir = "/opt/yascan";
            let file_path = Path::new("/opt/yascan2/file.exe");
            let program_dir_path = Path::new(program_dir);
            // This should NOT match because yascan2 is different from yascan
            assert!(!file_path.starts_with(program_dir_path));
        }
        ...
        fn test_program_dir_nested_file() {
            let program_dir = "/usr/local/yascan";
            let file_path = Path::new("/usr/local/yascan/config/excludes.cfg");
            let program_dir_path = Path::new(program_dir);
            assert!(file_path.starts_with(program_dir_path));
        }
    }
