...
pub fn scan_processes(
    ...
    // Set total process count for progress tracking
    if let Some(state) = scan_state {
        state.set_files_total(sys.processes().len());
    }
...
fn process_single_process(
    ...
    // Skip if no rules compiled (avoid crash with empty rules)
    if compiled_rules.iter().count() == 0 {
        return (processes_scanned, processes_matched, alert_count, warning_count, notice_count);
    }
            ...
            for matching_rule in scan_results.matching_rules() {
                if !proc_matches.is_full() {
                    ...
                    proc_matches.insert(
                        ...
                        rule_file: if rule_file.is_empty() { None } else { Some(rule_file) },
                        rule_path: None,
                    }
                    );
                }
    ...
    if !proc_matches.is_empty() {
        ...
        let shown_reasons: Vec<MatchReason> = proc_matches.iter().take(reasons_to_show)
            .map(|r| MatchReason { 
                ...
                rule_file: r.rule_file.clone(),
                rule_path: r.rule_path.clone(),
            })
            .collect();
