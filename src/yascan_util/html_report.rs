//! HTML Report Generation from JSONL files for yascan-util
//! 
//! Provides functionality to generate HTML reports from single or multiple JSONL files,
//! reusing the existing HTML rendering pipeline from yascan.
...
// Duplicate types from helpers/html_report.rs since yascan-util is a separate binary
...
// ScanConfig duplicate (simplified for yascan-util)
...
pub fn extract_metadata(data: &ReportData, filename: &str) -> SourceMetadata {
    let hostname = data.scan_start.as_ref()
        ...
        .unwrap_or_else(|| {
            // Try to extract from filename (yascan_hostname_date.jsonl)
            Path::new(filename)
                .file_stem()
                .and_then(|s| s.to_str())
                .and_then(|s: &str| s.strip_prefix("yascan_"))
...
pub fn generate_single_report(
    ...
    let version = extract_version(&report_data, YASCAN_UTIL_VERSION);
...
pub fn render_combined_html(
    ...
    html.push_str("    <title>Yascan Combined Scan Report</title>\n");
    html.push_str("    <style>\n");
    ...
    html.push_str("        <h1>Yascan Combined Scan Report</h1>\n");
    ...
    html.push_str("        </div>\n");
    html.push_str("        <div class=\"stat-card\">\n");
    html.push_str("            <h3>Yascan Versions</h3>\n");
    ...
    html.push_str(r##"        // =====================================================
        ...
        // =====================================================
        const STORAGE_KEY = 'yascan_combined_filters_' + new Date().toISOString().slice(0, 10).replace(/-/g, '_');
        let filterList = [];
        let selectedText = '';
        ...
        function exportFilters() {
            ...
            a.href = url;
            a.download = 'yascan-combined-filters-' + new Date().toISOString().slice(0, 10) + '.json';
            a.click();
...
const YASCAN_UTIL_VERSION: &str = env!("CARGO_PKG_VERSION");
...
/// Simplified render_html function for yascan-util
/// This produces HTML reports similar to the main yascan binary
fn render_html_simplified(
    ...
    let default_title = format!("Yascan Scan Report - {}", hostname);
...
<body>
    <header>
        <div class="logo-text">Yascan <span class="version">v{}</span></div>
        <div class="scan-info">
            <div class="info-card">
                <h3>Hostname</h3>
                <p>{}</p>
