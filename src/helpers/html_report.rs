...
<head>
    ...
    <title>Yascan Scan Report - {hostname}</title>
    <style>
        :root {{
            --bg-primary: #000000;
            --bg-secondary: #0a0a0a;
        ...
        * {{
        ...
        .stat-dot.alert {{ background: #ff0000; }}
        .stat-dot.warning {{ background: #cc9900; }}
        .stat-dot.notice {{ background: #00ffff; }}
        ...
        .score-bar.alert {{ background: #ff0000; }}
        .score-bar.warning {{ background: #cc9900; }}
        .score-bar.notice {{ background: #00ffff; }}
...
<body>
    ...
    <script>
        ...
        // =====================================================
        const STORAGE_KEY = 'yascan_filters_' + '{jsonl_filename}'.replace(/[^a-zA-Z0-9]/g, '_');
        let filterList = [];
        let selectedText = '';
        ...
        function exportFilters() {{
            ...
            a.href = url;
            a.download = 'yascan-filters-' + new Date().toISOString().slice(0, 10) + '.json';
            a.click();
...
fn render_finding_card(finding: &LogEvent, idx: usize) -> String {
    ...
    let reasons_html = if let Some(ref reasons) = finding.reasons {
        ...
        for reason in sorted_reasons {
            ...
            if let Some(ref rule_file) = reason.rule_file {
                reasons_str.push_str(&format!(
                    r#"<span><strong>Rule File:</strong> {}</span>"#,
                    html_escape(rule_file)
                ));
            }
            
            if let Some(ref rule_path) = reason.rule_path {
                reasons_str.push_str(&format!(
                    r#"<span><strong>Rule Path:</strong> {}</span>"#,
                    html_escape(rule_path)
                ));
            }
    ...
    format!(
        r#"<div class="finding-card" data-level="{level_class}" id="finding-{idx}">
            <div class="finding-header">
                ...
                <div class="score-bar-wrap"><div class="score-bar {level_class}" style="width:{score}%"></div></div>
