...
# Common functions for Yascan test suite
# 
...
setup_temp_dir() {
    TEST_TEMP_DIR=$(mktemp -d -t yascan_test_XXXXXX)
...
run_yascan() {
    ...
    if [ "$has_folder_flag" = false ]; then
        echo -e "${RED}ERROR: run_yascan MUST include -f <folder> to prevent full system scans${NC}" >&2
        echo "Usage: run_yascan -f \"\$TEST_TEMP_DIR\" [other args...]" >&2
        TEST_OUTPUT="ERROR: Missing -f flag"
        return 1
    fi
    
    TEST_OUTPUT=$("$project_root/build/yascan" --no-tui --no-html --no-log --no-jsonl "$@" 2>&1) || return $?
    return 0
}

# Run yascan for info/help commands only (no scanning)
# Usage: run_yascan_info --help | --version
# Returns: Sets TEST_OUTPUT and returns exit code
run_yascan_info() {
    local project_root=$(get_project_root)
    ...
    if [ "$is_safe" = false ]; then
        echo -e "${RED}ERROR: run_yascan_info only allows --help or --version${NC}" >&2
        echo "For scanning, use: run_yascan -f \"\$TEST_TEMP_DIR\" [args...]" >&2
        TEST_OUTPUT="ERROR: Invalid command for run_yascan_info"
        return 1
    fi
    
    TEST_OUTPUT=$("$project_root/build/yascan" "$@" 2>&1) || return $?
    return 0
}

# Run yascan-util
# Usage: run_yascan_util [args...]
run_yascan_util() {
    local project_root=$(get_project_root)
    TEST_OUTPUT=$("$project_root/build/yascan-util" "$@" 2>&1) || return $?
    return 0
}
