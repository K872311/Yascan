#!/bin/bash
# Tests for yascan-util HTML report generation
...
# Find yascan-util binary
if [ -f "$BUILD_DIR/yascan-util" ]; then
    YASCAN_UTIL="$BUILD_DIR/yascan-util"
elif [ -f "$PROJECT_ROOT/target/debug/yascan-util" ]; then
    YASCAN_UTIL="$PROJECT_ROOT/target/debug/yascan-util"
else
    echo "Error: yascan-util binary not found. Please build it first."
    exit 1
fi

echo "Using yascan-util: $YASCAN_UTIL"
...
test_single_jsonl_to_html() {
    ...
    if ! "$YASCAN_UTIL" html --input "$input" --output "$output"; then
    ...
    if ! grep -q "Yascan Scan Report" "$output"; then
...
test_combined_report() {
    ...
    if ! "$YASCAN_UTIL" html --input "$input_pattern" --combine --output "$output"; then
        echo "FAIL: Command failed"
        return 1
    fi
...
test_default_output_path() {
    ...
    if ! "$YASCAN_UTIL" html --input "$input"; then
        echo "FAIL: Command failed"
        return 1
    fi
