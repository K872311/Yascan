...
# Test Runner: Run All Yascan Tests
...
echo -e "${BLUE}=== Running All Yascan Tests ===${NC}"
echo ""
...
if [ ! -f "./build/yascan" ]; then
    echo -e "${RED}ERROR: build/yascan not found. Please run 'make package' first.${NC}"
    exit 1
fi
if [ ! -f "./build/yascan-util" ]; then
    echo -e "${RED}ERROR: build/yascan-util not found. Please run 'make package' first.${NC}"
    exit 1
fi
